...
public:
    ...
    char *getDetials() const override{
        char *result = new char[50];
        sprintf(result, "行号：%d, 类型：%s", line, type);
        return result;
    }
...
class ArraySubscriptExpr : public BinaryOperator{
public:
    ArraySubscriptExpr(unsigned line, const std::string& str1, const std::string& _str2, const std::string& _type)
    :BinaryOperator(line, str1, "", _str2, _type){}
    char *getAsString() const override{
        char *result = new char[strlen(str) + strlen(str2) + 3];
        sprintf(result, "%s[%s]", str, str2);
        return result;
    }
};
...
int main(){
    ...
    lines.push_back(std::make_unique<ArraySubscriptExpr>(3, "adsf", "aadfh", "int"));
